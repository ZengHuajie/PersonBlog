<template>
  <div class="container">
    <h2>文章编辑</h2>
    <el-input class="blog-title" v-model="blogTitle" placeholder="请输入文章标题"></el-input>
    <el-input class="blog-tags" v-model="blogTags" placeholder="请输入文章标签, 并以逗号隔开"></el-input>
  ...
  export default {
    ...
    methods: {
      handleSubmit() {
        ...
        console.log(`标题: ${blogTitle}, 标签: ${blogTags}, 内容: ${blogContent}`);
        this.$axios.post('/api/blogEdit', {
          blogTitle,
          blogTags,
          blogContent
        }).then((res) => {
          console.log('博客插入成功, 博客的ID为-->', res);
        })
        // this.blogTitle = '';
        // this.blogTags = '';
